...
app.get('/', (req, res) => {
  ...
  res.setHeader('Content-Security-Policy', `script-src 'self' 'nonce-${nonce}'; style-src 'nonce-${nonce}'; object-src 'none'; img-src 'self';`);
...
app.post('/', (req, res) => {
  ...
  if (targetUrl) {
    if (Date.now() - last_request_time < TIMEOUT) {
      responseText = `Error: You can only submit 1 post a minute. Wait ${Math.floor((TIMEOUT - (Date.now() - last_request_time))/1000)} seconds.`;
    } else {
