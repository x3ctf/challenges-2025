...
func (e *exportsRepository) GetExportImage(ctx context.Context, tenantID string, id int64) ([]byte, error) {
	var image []byte
	err := e.db.QueryRowContext(ctx, "SELECT image FROM exports WHERE id = $1 AND tenant_id = $2", id, tenantID).Scan(&image)
	if err != nil {
...
func (e *exportsRepository) GetAllExports(ctx context.Context, tenantID string) ([]*Export, error) {
	rows, err := e.db.QueryContext(ctx, "SELECT id, story_id, dimensions, progress, ready FROM exports WHERE tenant_id = $1", tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (e *exportsRepository) GetExport(ctx context.Context, tenantID string, id int64) (*Export, error) {
	var row Export
	err := e.db.QueryRowContext(ctx, "SELECT id, story_id, dimensions, progress, ready FROM exports WHERE id = $1 AND tenant_id = $2", id, tenantID).Scan(&row.ID, &row.StoryID, &row.Dimensions, &row.Progress, &row.Ready)
	if err != nil {
		return nil, err
	}
	return &row, nil
