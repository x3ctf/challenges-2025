...
func (s *storiesRepository) GetStory(ctx context.Context, tenantID string, id int64) (*Story, error) {
	var story Story
	err := s.db.QueryRowContext(ctx, "SELECT id, text, action, image_id FROM stories WHERE id = $1 AND tenant_id = $2 LIMIT 1", id, tenantID).Scan(&story.ID, &story.Text, &story.Action, &story.ImageID)
...
func (s *storiesRepository) GetAllStories(ctx context.Context, tenantID string) ([]*Story, error) {
	rows, err := s.db.QueryContext(ctx, "SELECT id, text, action, image_id FROM stories WHERE tenant_id = $1", tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
